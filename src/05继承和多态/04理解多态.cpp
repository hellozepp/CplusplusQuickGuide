...
在继承结构中，基类指针或引用指向派生类对象，通过该指针或引用调用同名覆盖方法（即虚函数）
，基类指针指向哪个派生类对象，就会调用哪个派生类对象的同名覆盖方法（override），称为多态
 pbase->bark();
 原理：多态底层是通过动态绑定来实现的，pbase->访问谁的vfptr=>继续访问谁的vftable
                                =>当然调用的是对应的派生类对象的方法了
...
// 动物的基类，此时动物不是一个具体的实体，不能用来实例化对象，所以定义为抽象类
class Animal
{
public:
