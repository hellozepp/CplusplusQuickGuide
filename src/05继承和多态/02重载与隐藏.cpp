...
2.隐藏(作用域的隐藏)的关系
在继承结构当中，派生类的同名成员，把基类的同名成员给隐藏调用了，隐藏不看参数列表


============================================================
1.方便描述，这里文案把继承结构，也说成从上（基类）到下（派生类）的结构
...
// 1.重载和隐藏
	...
	d.Base::show();
    // d.show(10)会报如下错误
	//“Derive::show”: 函数不接受 1 个参数，因为派生类把基类同名的show函数给隐藏了，如果想调用要加上作用域
...
// 基类到派生类对象的转换
    ...
    // Derive *pd = &b; // 报错，直接写不支持从上到下的转换，因为派生类对象的内存空间比基类对象大，此时指针操作很危险，超出base范围的内存访问是不安全的
	Derive *pd = (Derive*)&b; // 不安全，涉及了内存的非法访问！
	pd->show(); // 此时调用派生类的show方法正常了，实际上根本没有mb成员变量，访问mb会出现问题
#endif
	return 0;
}
